...
int main()
{
	...
	//Output
	uint32_t seg0, seg1, seg2, seg3;
	uint32_t done;

	done = Xil_In32(XPAR_CUSTOMIPCORE_0_S00_AXI_BASEADD + 16);

	do{
		seg0 = Xil_In32(XPAR_CUSTOMIPCORE_0_S00_AXI_BASEADD + 0);
		seg1 = Xil_In32(XPAR_CUSTOMIPCORE_0_S00_AXI_BASEADD + 4);
		seg2 = Xil_In32(XPAR_CUSTOMIPCORE_0_S00_AXI_BASEADD + 8);
		seg3 = Xil_In32(XPAR_CUSTOMIPCORE_0_S00_AXI_BASEADD + 12);
	}while(done = 1);

	Xil_printf("%d and %d and %d and %d", seg0, seg1, seg2, seg3, seg4);

	// Time to read the result from hardware
	timeElapsed = StopAndGetPerformanceTimer();
	xil_printf("\n\rRead the output", timeElapsed / (XPAR_CPU_M_AXI_DP_FREQ_HZ / 1000000));
	PrintDataArray(dstData, min(8, N));
